...
<div class="card border-0 shadow-lg" style="background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%); border-radius: 12px;">
    <div class="card-header border-0" style="background: rgba(255, 255, 255, 0.08); border-radius: 12px 12px 0 0; padding: 1.25rem;">
        <h6 class="mb-0" style="color: #ffffff; font-weight: 700; text-transform: uppercase; font-size: 0.9rem; letter-spacing: 0.5px;">
            <i class="fas fa-list me-2"></i>Data Tabel Jurusan
        </h6>
    </div>
                ...
                <table class="table table-striped mb-0" id="dataTable" style="min-width: 700px;">
                    <thead>
                        <tr class="text-center" style="background: rgba(255, 255, 255, 0.08);">
                            ...
                            <th style="color: #ffffff; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; font-size: 0.75rem; padding: 1rem; border-bottom: 2px solid #404040; min-width: 150px;">Kode Jurusan</th>
                            <th style="color: #ffffff; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; font-size: 0.75rem; padding: 1rem; border-bottom: 2px solid #404040; min-width: 200px;">Nama Jurusan</th>
                    ...
                    <tbody>
                        ...
                        <tr class="text-center" style="border-bottom: 1px solid #404040; transition: all 0.3s ease;" onMouseEnter="this.style.backgroundColor='rgba(255, 255, 255, 0.05)';" onMouseLeave="this.style.backgroundColor='transparent';">
                            ...
                            <td style="color: #ffffff;padding: 0.9rem; font-weight: 600;">{{ $item->kode_jurusan }}</td>
                            <td style="color: #ffffff;padding: 0.9rem; text-align:">{{ $item->nama_jurusan }}</td>
                            <td style="color: #ffffff;padding: 0.9rem; font-size: 0.9rem;">{{ $item->created_at->format('d/m/Y') }}</td>
                            @if(Auth::user()->role !== 'siswa')
                            <td style="padding: 0.9rem;">
                                <div class="btn-group gap-2" role="group">
                                    <a href="{{ route('jurusan.edit', $item->id) }}" class="btn btn-sm" style="background: rgba(234, 179, 8, 0.15); color: #eab308; border: 1px solid #eab308; border-radius: 6px; padding: 0.4rem 0.7rem; transition: all 0.3s ease;" onMouseEnter="this.style.background='rgba(234, 179, 8, 0.25)';" onMouseLeave="this.style.background='rgba(234, 179, 8, 0.15)';">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('jurusan.destroy', $item->id) }}" method="POST" class="d-inline delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-sm delete-btn" style="background: rgba(239, 68, 68, 0.15); color: #ef4444; border: 1px solid #ef4444; border-radius: 6px; padding: 0.4rem 0.7rem; transition: all 0.3s ease;" onMouseEnter="this.style.background='rgba(239, 68, 68, 0.25)';" onMouseLeave="this.style.background='rgba(239, 68, 68, 0.15)';" data-item-name="{{ $item->nama_jurusan }}">
                        ...
                        @empty
                        <tr>
                            <td colspan="{{ Auth::user()->role !== 'siswa' ? 5 : 4 }}" class="text-center py-4" style="color: #b0b0b0;">Tidak ada data jurusan</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            Swal.fire({
                ...
                html: `Jurusan <strong>"${itemName}"</strong> akan dihapus permanen!`,
                icon: "warning",

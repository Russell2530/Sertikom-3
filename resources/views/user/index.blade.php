    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div>
            <div class="d-flex align-items-center gap-3 mb-2">
                <h1 style="font-size: 3rem; font-weight: 700; color: #ffffff; margin: 0;">Manajemen User</h1>
            </div>
            <p style="color: #b0b0b0; margin: 0; font-size: 0.9rem;">Kelola semua user dan permission</p>
        </div>
...
<div class="card border-0 shadow-lg" style="background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%); border-radius: 12px;">
    <div class="card-header border-0" style="background: rgba(255, 255, 255, 0.08); border-radius: 12px 12px 0 0; padding: 1.25rem;">
        <h6 class="mb-0" style="color: #ffffff; font-weight: 700; text-transform: uppercase; font-size: 0.9rem; letter-spacing: 0.5px;">
            <i class="fas fa-list me-2"></i>Data Tabel User
        </h6>
    </div>
                ...
                <table class="table table-striped mb-0" id="dataTable" style="min-width: 800px;">
                    <thead>
                        <tr class="text-center" style="background: rgba(255, 255, 255, 0.08);">
                            ...
                            <th style="color: #ffffff; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; font-size: 0.75rem; padding: 1rem; border-bottom: 2px solid #404040; min-width: 150px;">Nama</th>
                            <th style="color: #ffffff; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; font-size: 0.75rem; padding: 1rem; border-bottom: 2px solid #404040; min-width: 200px;">Email</th>
                            <th style="color: #ffffff; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; font-size: 0.75rem; padding: 1rem; border-bottom: 2px solid #404040; min-width: 100px;">Role</th>
                    ...
                    <tbody>
                        ...
                        <tr class="text-center align-middle" style="border-bottom: 1px solid #404040; transition: all 0.3s ease;" onMouseEnter="this.style.backgroundColor='rgba(255, 255, 255, 0.05)';" onMouseLeave="this.style.backgroundColor='transparent';">
                            ...
                            <td style="color: #ffffff; padding: 0.9rem; text-align: left; font-weight: 500;">{{ $user->name }}</td>
                            <td style="color: #ffffff; padding: 0.9rem; font-size: 0.9rem;">{{ $user->email }}</td>
                            <td style="padding: 0.9rem;">
                                @if($user->role == 'admin')
                                    <span style="background: rgba(239, 68, 68, 0.2); color: #ef4444; padding: 0.4rem 0.8rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600;">Admin</span>
                                @elseif($user->role == 'guru')
                                    <span style="background: rgba(234, 179, 8, 0.2); color: #eab308; padding: 0.4rem 0.8rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600;">Guru</span>
                                @else
                                    <span style="background: rgba(34, 197, 94, 0.2); color: #22c55e; padding: 0.4rem 0.8rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600;">Siswa</span>
                                @endif
                            </td>
                            <td style="color: #ffffff; padding: 0.9rem; font-size: 0.9rem;">{{ $user->created_at->format('d/m/Y') }}</td>
                            <td style="padding: 0.9rem;">
                                <div class="btn-group gap-2" role="group">
                                    <a href="{{ route('users.edit', $user->id) }}" class="btn btn-sm" style="background: rgba(234, 179, 8, 0.15); color: #eab308; border: 1px solid #eab308; border-radius: 6px; padding: 0.4rem 0.7rem; transition: all 0.3s ease;" onMouseEnter="this.style.background='rgba(234, 179, 8, 0.25)';" onMouseLeave="this.style.background='rgba(234, 179, 8, 0.15)';">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            const roleText = userRole === 'admin' ? 'Admin' : (userRole === 'guru' ? 'Guru' : 'Siswa');
            
            Swal.fire({
                title: "Hapus User?",
                html: `User <strong>"${userName}"</strong> (${roleText}) akan dihapus permanen!<br><small class="text-muted">Semua data terkait user ini akan hilang.</small>`,
                icon: "warning",

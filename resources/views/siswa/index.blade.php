...
@section('content')
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            ...
            @if (Auth::user()->role !== 'siswa')
                <a href="{{ route('siswa.create') }}"
                    style="background: linear-gradient(135deg, #ffffff 0%, #e0e0e0 100%); color: #0a0a0a; font-weight: 600; border: none; padding: 0.8rem 1.8rem; border-radius: 10px; text-decoration: none; display: inline-flex; align-items: center; gap: 0.5rem; transition: all 0.3s ease;"
    ...
    <!-- Search & Filter -->
    @if (Auth::user()->role !== 'siswa')
        <div class="card mb-5 border-0 shadow-lg mt-3"
            style="background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%); border-radius: 12px;">
            <div class="card-body p-4">
                <form action="{{ route('siswa.index') }}" method="GET" class="mb-5">
                    <div class="row g-3">
                        <div class="col-md-4 col-sm-12">
                            <div class="input-group">
                                <input type="text" name="search" class="form-control"
                                    placeholder="Cari NISN atau nama..." value="{{ request('search') }}"
                                    style="background: #2d2d2d; border: 1px solid #404040; color: #ffffff; border-radius: 8px;">
                                <button class="btn btn-outline-secondary" type="submit"
                                    style="border-color: #404040; color: #ffffff;">
                        ...
                        <div class="col-md-3 col-sm-12">
                            <select name="jurusan_id" class="form-select"
                                style="background: #2d2d2d; border: 1px solid #404040; color: #ffffff; border-radius: 8px;">
                                <option value=""
                                    style="background: #2d2d2d; border: 1px solid #404040; color: #ffffff;">Semua Jurusan
                                </option>
                                @foreach ($jurusan as $j)
                                    <option value="{{ $j->id }}"
                                        style="background: #2d2d2d; border: 1px solid #404040; color: #ffffff; border-radius:"
                        ...
                        <div class="col-md-3 col-sm-12">
                            <select name="kelas_id" class="form-select"
                                style="background: #2d2d2d; border: 1px solid #404040; color: #ffffff; border-radius: 8px;">
                                <option value=""
                                    style="background: #2d2d2d; border: 1px solid #404040; color: #ffffff;">Semua Kelas
                                </option>
                                @foreach ($kelas as $k)
                                    <option value="{{ $k->id }}"
                                        style="background: #2d2d2d; border: 1px solid #404040; color: #ffffff;"
                        ...
                        <div class="col-md-2 col-sm-12">
                            <button type="submit" class="btn btn-primary w-100"
                                style="background: linear-gradient(135deg, #ffffff 0%, #e0e0e0 100%); color: #1a1a1a; font-weight: 600; border: none;">Filter</button>
                            @if (request()->hasAny(['search', 'jurusan_id', 'kelas_id']))
                                <a href="{{ route('siswa.index') }}" class="btn btn-secondary w-100 mt-2"
                                    style="background: #2d2d2d; border: 1px solid #404040; color: #ffffff; border-radius: 8px;">
                                    Reset
                                </a>
                            @endif
                    ...
                    <table class="table table-striped mb-0" id="dataTable" style="min-width: 1100px;">
                        <thead>
                            <tr class="text-center" style="background: rgba(255, 255, 255, 0.08);">
                                <th
                                    style="color: #ffffff; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; font-size: 0.75rem; padding: 1rem; border-bottom: 2px solid #404040;">
                                    #</th>
                                <th
                                    style="color: #ffffff; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; font-size: 0.75rem; padding: 1rem; border-bottom: 2px solid #404040;">
                                    Foto</th>
                        ...
                        <tbody>
                            @forelse($siswa as $item)
                                <tr class="text-center align-middle"
                                    style="border-bottom: 1px solid #404040; transition: all 0.3s ease;"
                                    onMouseEnter="this.style.backgroundColor='rgba(255, 255, 255, 0.05)';"
                                    onMouseLeave="this.style.backgroundColor='transparent';">
                                    <td style="color: #ffffff;  padding: 0.9rem;">
                                        {{ $loop->iteration + ($siswa->currentPage() - 1) * $siswa->perPage() }}</td>
                                    <td style="padding: 0.9rem;">
                                        @if ($item->foto)
                                            <img src="{{ asset('storage/' . $item->foto) }}" alt="Foto"
                                                class="rounded-circle" width="45" height="45"
                                                style="object-fit: cover; border: 2px solid #ffffff; box-shadow: 0 2px 8px rgba(255, 255, 255, 0.2);">
                                        @else
                                            <div class="rounded-circle d-flex align-items-center justify-content-center mx-auto"
                                                style="width: 45px; height: 45px; background: linear-gradient(135deg, #ffffff 0%, #e0e0e0 100%); box-shadow: 0 2px 8px rgba(255, 255, 255, 0.2);">
                                    ...
                                    <td style="color: #ffffff; padding: 0.9rem;">{{ $item->tahunAjar->nama_tahun_ajar }}
                                    </td>
                                    @if (Auth::user()->role !== 'siswa')
                                        <td style="padding: 0.9rem;">
                                            <div class="btn-group gap-2" role="group">
                                                <a href="{{ route('siswa.show', $item->id) }}" class="btn btn-sm"
                                                    title="Lihat"
                                                    style="background: white; border: 1px solid #404040; border-radius: 6px; padding: 0.4rem 0.7rem; transition: all 0.3s ease;"
                                                    onMouseEnter="this.style.background='white'; this.style.borderColor='#ffffff';"
                                                    onMouseLeave="this.style.background='white'; this.style.borderColor='#404040';">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <a href="{{ route('siswa.edit', $item->id) }}" class="btn btn-sm"
                                                    title="Edit"
                                                ...
                                                <form action="{{ route('siswa.destroy', $item->id) }}" method="POST"
                                                    ...
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-sm delete-btn" title="Hapus"
                                                        ...
                                                        data-item-name="{{ $item->nama_lengkap }}">
                                    ...
                                    @else
                                        <td style="padding: 0.9rem;">
                                            <a href="{{ route('siswa.show', $item->id) }}" class="btn btn-sm"
                                                style="background: rgba(255, 255, 255, 0.1); color: #ffffff; border: 1px solid #404040; border-radius: 6px; padding: 0.4rem 0.7rem; text-decoration: none;">
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ Auth::user()->role !== 'siswa' ? 8 : 7 }}" class="text-center py-4"
                                        style="color: #b0b0b0;">Tidak ada data siswa</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            ">
                ...
                <div class="scroll-controls mt-3" style="display: flex; justify-content: center; gap: 10px;">
                    <button class="btn btn-sm scroll-btn" data-direction="left"
                        style="border-color: #404040; color: #e0e0e0; border-radius: 8px; background: transparent; transition: all 0.3s ease;"
